...
uint8_t iistxbuff[(BUFF_NUM +1) * SINGLE_BUFF_SIZE], iisrxbuff[256]; //i2semptybuff[EP3_MAX_PKT_SIZE];
uint32_t usb_read_pos = 0, iis_send_pos = 0, buff_size = BUFF_NUM * SINGLE_BUFF_SIZE;
uint16_t i2semptybuff[EP3_MAX_PKT_SIZE/2];
uint8_t audiofeedbackbuffer[3] = {0xc0,0x00,0x00};		//48000
...
//-----------------------------------------------------------------------
  ...
  //-------------------------------------------------------
  // word transfer
  DMAVEC->CTRL[IISTX_DMA_CHANNEL].DMACR = ((1 << FIFO_Pos) | (1 << SZ_Pos) | (0 << DAMOD_Pos) | (1 << SAMOD_Pos) | (0 << RPTSEL_Pos) | (0 << MODE_Pos) | (0 << RPTINT_Pos)); 
  DMAVEC->CTRL[IISTX_DMA_CHANNEL].DMBLS = 2;
  DMAVEC->CTRL[IISTX_DMA_CHANNEL].DMACT = (sizeof(iistxbuff) - SINGLE_BUFF_SIZE)/(2*2); 	//keep a buff empty
  DMAVEC->CTRL[IISTX_DMA_CHANNEL].DMRLD = (sizeof(iistxbuff) - SINGLE_BUFF_SIZE)/(2*2);  
  DMAVEC->CTRL[IISTX_DMA_CHANNEL].DMSAR = (uint32_t)((uint16_t*)&iistxbuff[0]);
//  DMAVEC->CTRL[IISTX_DMA_CHANNEL].DMACT = sizeof(i2semptybuff)/(2*2);
//  DMAVEC->CTRL[IISTX_DMA_CHANNEL].DMRLD = sizeof(i2semptybuff)/(2*2);
...
//	DMA_Enable(DMA_VECTOR_SSI_RX);
  ...
  INTC_EnableIRQ(SSIDMATX_IRQn);
	...
	NVIC_SetPriority (SSIDMATX_IRQn%32, 1);	//laidi
	NVIC_SetPriority (USBI_IRQn, 3);
	NVIC_SetPriority (USBR_IRQn, 3);
  SSI_Open(SSI_CH0);
  SSI_Start(SSI_CH0);
//while(1)
//{}
