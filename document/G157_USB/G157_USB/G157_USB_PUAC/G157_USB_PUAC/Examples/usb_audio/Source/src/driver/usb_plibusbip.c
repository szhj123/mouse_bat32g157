...
#include "usb_reg_access.h"
    ...
    #include "usb_puac_mini_config.h"
...
uint8_t usb_pstd_set_pipe_table (uint8_t *p_descriptor)
{
    ...
    switch ((uint16_t)(p_descriptor[USB_EP_B_ATTRIBUTES] & USB_EP_TRNSMASK))
    {
            ...
            {
                /* IN(send) */
                pipe_no     = usb_pstd_get_pipe_no (USB_EP_ISO, USB_PIPE_DIR_IN);
                pipe_cfg    = (uint16_t)(USB_TYPFIELD_ISO | USB_CFG_DBLB | USB_DIR_P_IN);
            }
            else
            {
                /* OUT(receive) */
                pipe_no     = usb_pstd_get_pipe_no (USB_EP_ISO, USB_PIPE_DIR_OUT);
                pipe_cfg    = (uint16_t)(USB_TYPFIELD_ISO | USB_CFG_DBLB | USB_DIR_P_OUT);
            }
...
uint8_t         usb_pstd_get_pipe_no (uint8_t type, uint8_t dir)
{
    ...
    if (USB_EP_ISO == type)
    {
        ...
        {
            pipe_no     = USB_CFG_PUAC_ISO_IN;
        }
        else
        {
            pipe_no     = USB_CFG_PUAC_ISO_OUT;
        }
    }
