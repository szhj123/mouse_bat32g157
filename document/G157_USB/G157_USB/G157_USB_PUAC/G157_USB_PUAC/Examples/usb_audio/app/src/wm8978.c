...
//{
//	
////	u16 tmp = (reg << 9) | (val & 0x01FF); //把寄存器地址和寄存器的值组合成16比特值，高7位是寄存器地址，低9位是寄存器值
////	u8 *u8ptr = (u8 *)&tmp;
////	
////	IICA0_MasterSend((WM8978_ADDR<<1|0), u8ptr, sizeof(tmp), 20); //写WM8978寄存器地址+数据
//	
//	uint8_t regtmp[2];
//	u16 tmp = (reg << 9) | (val & 0x01FF); //把寄存器地址和寄存器的值组合成16比特值，高7位是寄存器地址，低9位是寄存器值
//	regtmp[0] = tmp>>8;
//	regtmp[1] = tmp&0xff;
//#if 0	
//	IICA0_MasterSend((WM8978_ADDR<<1|0), regtmp, sizeof(regtmp), 20); //写WM8978寄存器地址+数据
//	
//	while(g_iica0_tx_end == 0);	//等待IIC传输完成
//#else
//	IIC_Write_NByte(WM8978_ADDR<<1,regtmp,sizeof(regtmp));
//	
//#endif
...
u8 WM8978_Write_Reg(uint8_t u8addr, uint16_t u16data)
{
    /* Send START */
    ...
    SCI1->SCR11 |= _8000_SCI_TRANSMISSION;
		...
		SCI1->SIO11 = WM8978_ADDR<<1;//device address RW=0
		
		while(!INTC_GetPendingIRQ(IIC11_IRQn));
		INTC_ClearPendingIRQ(IIC11_IRQn);
		
		SCI1->SIO11 = (uint8_t)((u8addr << 1) | (u16data >> 8));//Registor
		
		while(!INTC_GetPendingIRQ(IIC11_IRQn));
		INTC_ClearPendingIRQ(IIC11_IRQn);
		
		SCI1->SIO11 = (uint8_t)(u16data & 0x00FF);//Data
    ...
    IIC11_StopCondition();
		...
		WM8978_REGVAL_TBL[u8addr]=u16data;	//保存寄存器值到本地
		return 0;	
