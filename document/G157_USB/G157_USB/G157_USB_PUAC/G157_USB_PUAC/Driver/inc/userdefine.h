...
#define USB_D0FIFO_DMA_CHANNEL 4
#define USB_D1FIFO_DMA_CHANNEL 5

#define UART0_TX_DMA_CHANNEL 6
...
//}while(0)
#define TO00_PORT_SETTING() do{ \
        PORT->PA00CFG = 0x01;	    /* allocate TO00 to PA00 */ \
        PORT->PCLRA =  (1 << 0);    /* PA00 output low level */ \
        PORT->PMA  &= ~(1 << 0);    /* PA00 is used as TO00 output */ \
        PORT->POMA &= ~(1 << 0);    /* PA00 is normal output mode */ \
        PORT->PMCA &= ~(1 << 0);    /* PA00 digital function */ \
}while(0)
//#define TO01_PORT_SETTING() do{ \
//        PORT->PC09CFG = 0x02;	    /* allocate TO01 to PC09 */ \
...
//        PORT->PC10CFG = 0x03;	    /* allocate TO02 to PC10 */ \
...
//        PORT->PC11CFG = 0x04;	    /* allocate TO03 to PC11 */ \
...
//}while(0)
#define TO01_PORT_SETTING() do{ \
        PORT->PD07CFG = 0x02;	    /* allocate TO01 to PD07 */ \
        PORT->PCLRD =  (1 << 7);    /* PD07 output low level */ \
        PORT->PMD  &= ~(1 << 7);    /* PD07 is used as TO00 output */ \
        PORT->POMD &= ~(1 << 7);    /* PD07 is normal output mode */ \
        PORT->PMCD &= ~(1 << 7);    /* PD07 digital function */ \
}while(0)

#define TO02_PORT_SETTING() do{ \
        PORT->PD08CFG = 0x03;	    /* allocate TO02 to PD08 */ \
        PORT->PCLRD =  (1 << 8);   /* PD08 output low level */ \
        PORT->PMD  &= ~(1 << 8);   /* PD08 is used as TO02 output */ \
        PORT->POMD &= ~(1 << 8);   /* PD08 is normal output mode */ \
        PORT->PMCD &= ~(1 << 8);   /* PD08 digital function */ \
}while(0)

#define TO03_PORT_SETTING() do{ \
        PORT->PD09CFG = 0x04;	    /* allocate TO03 to PD09 */ \
        PORT->PCLRD =  (1 << 9);   /* PD09 output low level */ \
        PORT->PMD  &= ~(1 << 9);   /* PD09 is used as TO03 output */ \
...
#define TI10_PORT_SETTING() do{ \
        PORT->TI10PCFG = 0x11;	    /* allocate TI10 to PD12 */ \
...
#define TI11_PORT_SETTING() do{ \
        PORT->TI11PCFG = 0x12;	    /* allocate TI11 to PD13 */ \
...
#define TI12_PORT_SETTING() do{ \
        PORT->TI12PCFG = 0x13;	    /* allocate TI12 to PD14 */ \
...
#define TI13_PORT_SETTING() do{ \
        PORT->TI13PCFG = 0x14;	    /* allocate TI13 to PD15 */ \
...
#define TO10_PORT_SETTING() do{ \
        PORT->PD12CFG = 0x01;	    /* allocate TO10 to PD12 */ \
...
#define TO11_PORT_SETTING() do{ \
        PORT->PD13CFG = 0x02;	    /* allocate TO11 to PD13 */ \
...
#define TO12_PORT_SETTING() do{ \
        PORT->PD14CFG = 0x03;	    /* allocate TO12 to PD14 */ \
...
#define TO13_PORT_SETTING() do{ \
        PORT->PD15CFG = 0x04;	    /* allocate TO13 to PD15 */ \
...
#define TI14_PORT_SETTING() do{ \
        PORT->TI14PCFG = 0x0C;	    /* allocate TI10 to PA11 */ \
...
#define TI15_PORT_SETTING() do{ \
        PORT->TI15PCFG = 0x0D;	    /* allocate TI11 to PA12 */ \
...
#define TI16_PORT_SETTING() do{ \
        PORT->TI16PCFG = 0x0E;	    /* allocate TI12 to PA13 */ \
...
#define TI17_PORT_SETTING() do{ \
        PORT->TI17PCFG = 0x0F;	    /* allocate TI13 to PA14 */ \
...
#define TO14_PORT_SETTING() do{ \
        PORT->PA11CFG = 0x01;	    /* allocate TO10 to PA11 */ \
...
#define TO15_PORT_SETTING() do{ \
        PORT->PA12CFG = 0x02;	    /* allocate TO11 to PA12 */ \
...
#define TO16_PORT_SETTING() do{ \
        PORT->PA13CFG = 0x03;	    /* allocate TO12 to PA13 */ \
...
#define TO17_PORT_SETTING() do{ \
        PORT->PA14CFG = 0x04;	    /* allocate TO13 to PA14 */ \
...
#define EPWMO00_PORT_SETTING() do{ \
        PORT->PD04CFG = 0x00;        /* PD04 default Alternate function */ \
...
#define EPWMO01_PORT_SETTING() do{ \
        PORT->PD05CFG = 0x00;        /* PD05 default Alternate function */ \
...
#define EPWMO02_PORT_SETTING() do{ \
        PORT->PD06CFG = 0x00;        /* PD06 default Alternate function */ \
...
#define EPWMO03_PORT_SETTING() do{ \
        PORT->PD07CFG = 0x00;        /* PD07 default Alternate function */ \
        PORT->PCLRD =  (1 << 7);    /* PD07 output low level */ \
...
#define EPWMO04_PORT_SETTING() do{ \
        PORT->PD08CFG = 0x00;        /* PD08 default Alternate function */ \
        PORT->PCLRD =  (1 << 8);    /* PD08 output low level */ \
...
#define EPWMO05_PORT_SETTING() do{ \
        PORT->PD09CFG = 0x00;        /* PD09 default Alternate function */ \
        PORT->PCLRD =  (1 << 9);    /* PD09 output low level */ \
...
#define EPWMO06_PORT_SETTING() do{ \
        PORT->PD10CFG = 0x00;        /* PD10 default Alternate function */ \
...
#define EPWMO07_PORT_SETTING() do{ \
        PORT->PD11CFG = 0x00;        /* PD11 default Alternate function */ \
...
#define CLKBUZ0_PORT_SETTING() do{ \
        PORT->PA00CFG = 0x00;       /* allocate CLKBUZ0 to PA00 */ \
        PORT->PCLRA  =  (1 << 0);   /* PA00 output low level */ \
...
//        PORT->PB06CFG = 0x06;       /* allocate CLKBUZ1 to PB06 */ \
//        PORT->PCLRB =  (1 << 6);   /* PB06 output low level */ \
//        PORT->PMB   &= ~(1 << 6);   /* PB06 is used as CLKBUZ1 output */ \
//        PORT->PMCB  &= ~(1 << 6);   /* PB06 digital function */ \
//}while(0)
#define CLKBUZ1_PORT_SETTING() do{ \
        PORT->PD05CFG = 0x06;       /* allocate CLKBUZ1 to PD05 */ \
        PORT->PCLRD =  (1 << 5);   /* PD05 output low level */ \
        PORT->PMD   &= ~(1 << 5);   /* PD05 is used as CLKBUZ1 output */ \
        PORT->PMCD  &= ~(1 << 5);   /* PD05 digital function */ \
}while(0)
/* ================================================================================================================== */
/* ================                                        ADC                                       ================ */
...
//        PORT->PD10CFG = 0x05;        /* allocate TXD0 to PD10 */ \
...
//}while(0)
#define TXD0_PORT_SETTING() do{ \
        PORT->PD11CFG = 0x05;  /* allocate TXD0 to PD11 */ \
...
//        PORT->RXD0PCFG = 0x14;      /* allocate RXD0 to PD11 */ \
//        PORT->PMD  |=  (1 << 11);   /* PD11 is used as RXD0 input */ \
//        PORT->PMCD &= ~(1 << 11);   /* PD11 digital function */ \
//}while(0)
//#define TXD0_PORT_SETTING() do{ \
//        PORT->PC09CFG = 0x05;        /* allocate TXD0 to PC09 */ \
//        PORT->PSETC =  (1 << 9);    /* PC09 output high level */ \
//        PORT->PMC  &= ~(1 << 9);    /* PC09 is used as TXD0 output */ \
//        PORT->POMC &= ~(1 << 9);    /* PC09 is normal output mode */ \
//        PORT->PMCC &= ~(1 << 9);    /* PC09 digital function */ \
//}while(0)
...
/* ToDo: You can allocate the RXD0 to any desired pins with RXD0PCFG register */
#define RXD0_PORT_SETTING() do{ \
        PORT->RXD0PCFG = 0x08;      /* allocate RXD0 to PC08 */ \
        PORT->PMC  |=  (1 << 8);   /* PC08 is used as RXD0 input */ \
        PORT->PMCC &= ~(1 << 8);   /* PC08 digital function */ \
}while(0)
/**
...
#define SCLKO00_PORT_SETTING() do{ \
        PORT->PA11CFG = 0x00;        /* allocate SCLKO00 to PA11 */ \
...
#define SDO00_PORT_SETTING() do{ \
        PORT->PD10CFG = 0x05;        /* allocate SDO00 to PD10 */ \
        PORT->PSETD =  (1 << 10);    /* PD10 output high level */ \
        PORT->PMD  &= ~(1 << 10);    /* PD10 is used as TXD0 output */ \
        PORT->POMD &= ~(1 << 10);    /* PD10 is normal output mode */ \
        PORT->PMCD &= ~(1 << 10);    /* PD10 digital function */ \
...
#define SDI00_PORT_SETTING() do{ \
        PORT->SDI00PCFG = 0x17;     /* allocate SDI00 to PD11 */ \
        PORT->PMD  |=  (1 << 11);   /* PD11 is used as RXD0 input */ \
        PORT->PMCD &= ~(1 << 11);   /* PD11 digital function */ \
}while(0)
...
#define SCLKI01_PORT_SETTING() do{ \
        PORT->PA14CFG = 0x00;        /* PA14 default Alternate function */ \
...
#define SDO01_PORT_SETTING() do{ \
        PORT->PA12CFG = 0x00;        /* PA12 default Alternate function */ \
...
#define SDI01_PORT_SETTING() do{ \
        PORT->PA13CFG = 0x00;        /* PA13 default Alternate function */ \
...
#define TXD1_PORT_SETTING() do{ \
        PORT->PC06CFG = 0x05;        /* allocate TXD1 to PC06 */ \
        PORT->PSETC =  (1 << 0);    /* PC06 output high level */ \
        PORT->PMC  &= ~(1 << 0);    /* PC06 is used as TXD1 output */ \
        PORT->POMC &= ~(1 << 0);    /* PC06 is normal output mode */ \
        PORT->PMCC &= ~(1 << 0);    /* PC06 digital function */ \
}while(0)

/* ToDo: You can allocate the RXD1 to any desired pins with RXD1PCFG register */
#define RXD1_PORT_SETTING() do{ \
        PORT->RXD1PCFG = 0x0B;      /* allocate RXD1 to PC07 */ \
        PORT->PMC  |=  (1 << 1);    /* PC07 is used as RXD1 input */ \
        PORT->PMCC &= ~(1 << 1);    /* PC07 digital function */ \
}while(0)
...
#define SCLKI11_PORT_SETTING() do{ \
        PORT->PC03CFG = 0x00;        /* PC03 default Alternate function */ \
...
#define SDO11_PORT_SETTING() do{ \
        PORT->PC04CFG = 0x00;        /* PC04 default Alternate function */ \
...
#define SDI11_PORT_SETTING() do{ \
        PORT->PC05CFG = 0x00;        /* PC05 default Alternate function */ \
...
#define TXD2_PORT_SETTING() do{ \
        PORT->PC03CFG = 0x06;        /* allocate TXD2 to PC03 */ \
        PORT->PSETC =  (1 << 3);    /* PC03 output high level */ \
        PORT->PMC  &= ~(1 << 3);    /* PC03 is used as TXD2 output */ \
        PORT->POMC &= ~(1 << 3);    /* PC03 is normal output mode */ \
        PORT->PMCC &= ~(1 << 3);    /* PC03 digital function */ \
}while(0)
...
#define RXD2_PORT_SETTING() do{ \
        PORT->RXD2PCFG = 0x07;      /* allocate RXD2 to PC04 */ \
        PORT->PMC  |=  (1 << 4);    /* PC04 is used as RXD2 input */ \
        PORT->PMCC &= ~(1 << 4);    /* PC04 digital function */ \
}while(0)
...
#define SCLKI21_PORT_SETTING() do{ \
        PORT->PB03CFG = 0x00;        /* PB03 default Alternate function */ \
...
#define SDO21_PORT_SETTING() do{ \
        PORT->PB05CFG = 0x00;        /* PB05 default Alternate function */ \
...
#define SDI21_PORT_SETTING() do{ \
        PORT->PB04CFG = 0x00;        /* PB04 default Alternate function */ \
...
#define SCK0_PORT_SETTING() do{ \
        PORT->SPIHS0_SCKIPCFG = 0x01; /* SCK0 alternate to PC03 */ \
        PORT->PMC  |=  (1 << 3);      /* PC03 is used as SCK0 input */ \
        PORT->PMCC &= ~(1 << 3);      /* PC03 digital function */ \
}while(0)

#define MOSI0_PORT_SETTING() do{ \
        PORT->SPIHS0_SIPCFG = 0x02; /* MOSI0 alternate to PC04 */ \
        PORT->PMC  |=  (1 << 4);    /* PC04 is used as MOSI input */ \
        PORT->PMCC &= ~(1 << 4);    /* PC04 digital function */ \
}while(0)

#define MISO0_PORT_SETTING() do{ \
        PORT->PC05CFG = 0x08;       /* MISO0 alternate to PC05 */ \
...
#define SCK0_PORT_SETTING() do{ \
        PORT->PC03CFG = 0x06;       /* SCK0 alternate to PC03 */ \
...
#define MOSI0_PORT_SETTING() do{ \
        PORT->PC04CFG = 0x06;       /* MOSI0 alternate to PC04 */ \
...
#define MISO0_PORT_SETTING() do{ \
        PORT->SPIHS0_MICFG = 0x03;  /* MISO0 alternate to PC05 */ \
...
#define NSS1_PORT_SETTING() do{ \
        PORT->SPIHS1_NSSPCFG  = 0x06;  /* PB06 is used as NSS1 input */ \
...
//        PORT->SCLA0PCFG = 0x11;     /* allocate SCLA0 to PD08 */ \
//        PORT->PCLRD =  (1 << 8);    /* PD08 output low level */ \
...
//        PORT->SDAA0PCFG = 0x12;     /* allocate SDAA0 to PD09 */ \
//        PORT->PCLRD =  (1 << 9);    /* PD09 output low level */ \
...
#define SCLA1_PORT_SETTING() do{ \
        PORT->SCLA1PCFG = 0x13;     /* allocate SCLA1 to PD05 */ \
        PORT->PCLRD =  (1 << 5);    /* PD05 output low level */ \
...
#define SDAA1_PORT_SETTING() do{ \
        PORT->SDAA1PCFG = 0x14;     /* allocate SDAA1 to PD06 */ \
        PORT->PCLRD =  (1 << 6);    /* PD06 output low level */ \
...
#else
#define SCLA1_PORT_SETTING() do{ \
        PORT->SCLA1PCFG = 0x01;     /* allocate SCLA1 to PB01 */ \
        PORT->PCLRB =  (1 << 1);    /* PB01 output low level */ \
        PORT->PMB  &= ~(1 << 1);    /* PB01 is used as SCLA1 output */ \
        PORT->PUB  |=  (1 << 1);    /* PB01 pullup enable */ \
        PORT->POMB |=  (1 << 1);    /* PB01 is used as Open Drain output */ \
        PORT->PMCB &= ~(1 << 1);    /* PB01 digital function */ \
}while(0)

/* ToDo: You can allocate the SDAA1 to any desired pins with SDAA1PCFG register */
#define SDAA1_PORT_SETTING() do{ \
        PORT->SDAA1PCFG = 0x02;     /* allocate SDAA1 to PB02 */ \
        PORT->PCLRB =  (1 << 2);    /* PB02 output low level */ \
        PORT->PMB  &= ~(1 << 2);    /* PB02 is used as SDAA1 inout */ \
        PORT->PUB  |=  (1 << 2);    /* PB02 pullup enable */ \
        PORT->POMB |=  (1 << 2);    /* PB02 is used as Open Drain output */ \
